...
bool ChannelImpl::CheckForQueuedMessageOnChannel(amqp_channel_t channel) const {
  ...
  frame_queue_t::const_iterator it =
      std::find_if(m_frame_queue.begin(), m_frame_queue.end(),
                   std::bind(&ChannelImpl::is_method_on_channel, _1,
  ...
  it = std::find_if(it + 1, m_frame_queue.end(),
                    std::bind(&ChannelImpl::is_on_channel, _1, channel));
...
std::uint32_t ChannelImpl::ComputeBrokerVersion(
  ...
  std::uint32_t version_major = std::stoi(version_components[0]);
  std::uint32_t version_minor = std::stoi(version_components[1]);
  std::uint32_t version_patch = std::stoi(version_components[2]);
